...
public:
    ...
    */
    ...
    void buildSegmentTree(int index,int low,int high,vector<int>& array){
        ...
        segmentTreeArray[index] = max(segmentTreeArray[2*index+1], segmentTreeArray[2*index+2]);
    }
    ...
    complete overlap; [2,2], [3,4] lies entirely in range(2,4)
        - return segmentTree[index] (max value of that range)
    */
    ...
    void maxRangeQuery(int left,int right){
        int index=0;    // start
        int low=0;
