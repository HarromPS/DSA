...
{
public:
   void bfs_dijkstra(int src,int destination,vector<pair<int,int>> adj_weighted_list[],vector<int>& dist,vector<int>& parent){
        ...
        while(!pq.empty()){
            ...
            for(auto neigh: adj_weighted_list[node]){
                ...
                if(dist[n] > edge_weight + node_dist){
                    ...
                    // when found a shorted distance update the parent
                    parent[n]=node;
        ...
        // trace back the path 
        vector<int> ans;
        ans.push_back(destination); // start from the destination
...
void solve()
{
    ...
    Solution s;
    vector<int> dist(n+1,(int)1e9);    // distance array initially with max distance
    vector<int> parent(n+1,0);       // topo sort array

    // initially parent will be itself 
    for(int i=0;i<n+1;i++){
        parent[i]=i;
    }
    ...
    s.bfs_dijkstra(src,destination,adj_weighted_list,dist,parent);
